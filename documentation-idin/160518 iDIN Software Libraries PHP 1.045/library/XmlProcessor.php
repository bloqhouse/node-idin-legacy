...
class XmlProcessor {
    ...
    public function addSignature($xml) {
        $doc = new \DOMDocument();
        $doc->loadXML($xml);
        ...
        $key->passphrase = Configuration::defaultInstance()->MerchantCertificatePassword;
        $key->loadKey(Configuration::defaultInstance()->MerchantCertificate['pkey']);
        $signature->sign($key);
        $fingerprint = $this->getFingerprint(Configuration::defaultInstance()->MerchantCertificate['cert']);
    ...
    private function checkIdxSignature($doc) {
        $signature = new XMLSecurityDSig();
        ...
        $key->loadKey(Configuration::defaultInstance()->RoutingServiceCertificate);
