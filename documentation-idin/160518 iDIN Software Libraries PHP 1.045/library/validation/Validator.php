        ...
        foreach ($accRep->getAttributes() as $attrKey => $attrValue) {
            switch ($attrKey) {
                case SamlAttribute::$ConsumerBin:
                    self::checkByValue($attrValue, '[A-Za-z]{4}[[:print:]]{0,1020}');
                    break;
                ...
                case SamlAttribute::$ConsumerInitials:
                    self::checkByValue($attrValue, '[[:print:]]{1,20}');
                    break;
                case SamlAttribute::$ConsumerDateOfBirth:
                    self::checkByValue($attrValue, '^\\d\\d\\d\\d(0[1-9]|1[012])(0[1-9]|[12][0-9]|3[01])$');
                    break;
